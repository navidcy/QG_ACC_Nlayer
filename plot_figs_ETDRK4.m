...
%     colormap(gca,flipud(cbrewer('div','Spectral',64)));
    ...
    axis([0 Lx 0 Ly]/Ld)
    if plotb==1
        title(['$\beta_{' num2str(in) '}y + q_{' num2str(in) '}(x,y,t)$'],'interpreter','latex','fontsize',18)
    else
        title(['$ q_{' num2str(in) '}(x,y,t)$'],'interpreter','latex','fontsize',18)
    end
...
%     colormap(gca,flipud(cbrewer('div','Spectral',64)));
    ...
    axis([0 Lx 0 Ly]/Ld)
    if plotU==1
        title(['$-U_{' num2str(in) '}y + \psi_{' num2str(in) '}(x,y,t)$'],'interpreter','latex','fontsize',18)
    else
        title(['$\psi_{' num2str(in) '}(x,y,t)$'],'interpreter','latex','fontsize',18)
    end
end
...
if Nz>1
    ...
    ylabel(['$\frac1{2}U_b^2$'],'interpreter','latex','fontsize',18)
    xlabel(['$t$ [years]'],'interpreter','latex','fontsize',18)
    subplot(312)
    plot((1:it)*dt/year,KE(:,1:it),'linewidth',2);
    ylabel(['$\frac1{2}\langle|\nabla\psi_{n}|^2\rangle$'],'interpreter','latex','fontsize',18)
    xlabel(['$t$ [years]'],'interpreter','latex','fontsize',18)
    ...
    set(h,'Orientation','vertical','interpreter','latex','fontsize',14,'Location','northwest')
    ylabel(['$\frac{f_0^2}{2g''_n}\langle(\psi_{n+1}-\psi_{n})^2\rangle$'],'interpreter','latex','fontsize',18)
    xlabel(['$t$ [years]'],'interpreter','latex','fontsize',18)
else
    subplot(211)
    plot((1:it)*dt/year,KEUb(1:it),'linewidth',2);
